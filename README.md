...
> 已自动判断运行环境（依据 `CODEX_INTERNAL_ORIGINATOR_OVERRIDE`）：
> - **桌面 App**：第一行用 `-` 补到面板折行宽度，把会话信息挤到第二行。
>   宽度用 `CODEX_METER_LINE_W` 调节（默认 52；若在你窗口里对不齐，就改成该面板
>   能放下的一行字符数）。
> - **终端 TUI**：真正的两行对齐（等宽、保留换行）。
> `k`/`M`=千/百万、`¥`=元。

```
Prompt:  12.3k tok  ¥0.02  cache 96.4%----------------
